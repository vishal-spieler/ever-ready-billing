...
export async function getDb() {
  ...
  await dbInstance.exec(`
    ...
    CREATE TABLE IF NOT EXISTS purchase_orders (
      ...
      valid_until TEXT NOT NULL,
      client_id INTEGER NOT NULL,
      vendor_or_client TEXT NOT NULL DEFAULT 'CLIENT',
    ...
    CREATE TABLE IF NOT EXISTS uploaded_pos (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      client_id INTEGER NOT NULL,
      vendor_or_client TEXT NOT NULL DEFAULT 'CLIENT',
    ...
    CREATE INDEX IF NOT EXISTS idx_uploaded_pos_client_id ON uploaded_pos(client_id);
  `);
  ...
  if (settingsCount.count === 0) {
    await dbInstance.run(`
      INSERT INTO company_settings (id, name, address, phone, email, gstin, pan, bank_name, bank_branch, account_name, account_no, ifsc_code, login_username, login_password)
      VALUES (
        1,
  ...
  // Migrations for existing databases (live data considerations)
  try {
    await dbInstance.exec(`ALTER TABLE invoice_items ADD COLUMN uom TEXT DEFAULT NULL`);
  } catch (e) {}
  try {
    await dbInstance.exec(`ALTER TABLE invoice_items ADD COLUMN cgst_rate REAL DEFAULT 9`);
  } catch (e) {}
  try {
    await dbInstance.exec(`ALTER TABLE invoice_items ADD COLUMN sgst_rate REAL DEFAULT 9`);
  } catch (e) {}
  try {
    await dbInstance.exec(`ALTER TABLE invoices ADD COLUMN po_file_name TEXT DEFAULT NULL`);
  } catch (e) {}
  try {
    await dbInstance.exec(`ALTER TABLE invoices ADD COLUMN po_file_type TEXT DEFAULT NULL`);
  } catch (e) {}
  try {
    await dbInstance.exec(`ALTER TABLE invoices ADD COLUMN po_file_data TEXT DEFAULT NULL`);
  } catch (e) {}

  // Quotation & PO item level migrations
  try {
    await dbInstance.exec(`ALTER TABLE quotation_items ADD COLUMN uom TEXT DEFAULT NULL`);
  } catch (e) {}
  try {
    await dbInstance.exec(`ALTER TABLE quotation_items ADD COLUMN cgst_rate REAL DEFAULT 9`);
  } catch (e) {}
  try {
    await dbInstance.exec(`ALTER TABLE quotation_items ADD COLUMN sgst_rate REAL DEFAULT 9`);
  } catch (e) {}

  try {
    await dbInstance.exec(`ALTER TABLE purchase_order_items ADD COLUMN uom TEXT DEFAULT NULL`);
  } catch (e) {}
  try {
    await dbInstance.exec(`ALTER TABLE purchase_order_items ADD COLUMN cgst_rate REAL DEFAULT 9`);
  } catch (e) {}
  try {
    await dbInstance.exec(`ALTER TABLE purchase_order_items ADD COLUMN sgst_rate REAL DEFAULT 9`);
  } catch (e) {}

  try {
    await dbInstance.exec(`ALTER TABLE purchase_orders ADD COLUMN file_name TEXT DEFAULT NULL`);
  } catch (e) {}
  try {
    await dbInstance.exec(`ALTER TABLE purchase_orders ADD COLUMN file_type TEXT DEFAULT NULL`);
  } catch (e) {}
  try {
    await dbInstance.exec(`ALTER TABLE purchase_orders ADD COLUMN file_data TEXT DEFAULT NULL`);
  } catch (e) {}
