  ...
  if (isSupabaseAvailable()) {
    ...
    try {
      ...
      for (const item of items) {
        await db.run(
          `INSERT INTO invoice_items (invoice_id, sr, desc, hsn, qty, rate, total, uom, cgst_rate, sgst_rate)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            invoiceId,
            item.sr,
            ...
            item.total,
            item.uom || null,
            item.cgst_rate !== undefined ? item.cgst_rate : 9,
            item.sgst_rate !== undefined ? item.sgst_rate : 9
          ]
        );
      }
...
export async function dbCreateQuotation(quotation, items) {
  if (isSupabaseAvailable()) {
    ...
    try {
      ...
      for (const item of items) {
        await db.run(
          `INSERT INTO quotation_items (quotation_id, sr, desc, hsn, qty, rate, total, uom, cgst_rate, sgst_rate)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            quotId, item.sr, item.desc, item.hsn, item.qty, item.rate, item.total,
            item.uom, 
            item.cgstRate !== undefined ? item.cgstRate : (item.cgst_rate !== undefined ? item.cgst_rate : 9), 
            item.sgstRate !== undefined ? item.sgstRate : (item.sgst_rate !== undefined ? item.sgst_rate : 9)
          ]
        );
      }
...
export async function dbCreatePurchaseOrder(po, items) {
  if (isSupabaseAvailable()) {
    ...
    try {
      ...
      for (const item of items) {
        await db.run(
          `INSERT INTO purchase_order_items (purchase_order_id, sr, desc, hsn, qty, rate, total, uom, cgst_rate, sgst_rate)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [
...
export async function dbDeleteUploadedPO(id) {
  ...
  if (isSupabaseAvailable()) {
    ...
    const db = await getDb();
    await db.run('DELETE FROM uploaded_pos WHERE id = ?', [intId]);
    return true;
  }
}

export async function dbUpdateInvoicePO(id, fileName, fileType, fileData) {
  const intId = parseInt(id, 10);
  if (isSupabaseAvailable()) {
    const { data, error } = await supabase
      .from('invoices')
      .update({
        ...
        po_file_type: fileType,
        po_file_data: fileData
      })
      .eq('id', intId)
      .select()
      .single();
    ...
    const db = await getDb();
    await db.run(
      `UPDATE invoices SET po_file_name = ?, po_file_type = ?, po_file_data = ? WHERE id = ?`,
      [fileName, fileType, fileData, intId]
    );
    return true;
  }
}

export async function dbDeleteInvoicePO(id) {
  const intId = parseInt(id, 10);
  console.log('[dbDeleteInvoicePO] Parsed integer ID:', intId);
  if (isSupabaseAvailable()) {
    const { data, error } = await supabase
      .from('invoices')
      .update({
    ...
    const db = await getDb();
    console.log('[dbDeleteInvoicePO] Executing SQLite UPDATE query for ID:', intId);
    const res = await db.run(
      `UPDATE invoices SET po_file_name = NULL, po_file_type = NULL, po_file_data = NULL WHERE id = ?`,
      [intId]
    );
    console.log('[dbDeleteInvoicePO] SQLite run result:', res);
    return true;
  }
}

export async function dbUpdatePurchaseOrderFile(id, fileName, fileType, fileData) {
  const intId = parseInt(id, 10);
  if (isSupabaseAvailable()) {
    const { data, error } = await supabase
      .from('purchase_orders')
    ...
    const db = await getDb();
    await db.run(
      `UPDATE purchase_orders SET file_name = ?, file_type = ?, file_data = ? WHERE id = ?`,
      [fileName, fileType, fileData, intId]
    );
    return true;
  }
...
export async function dbDeletePurchaseOrderFile(id) {
  const intId = parseInt(id, 10);
  if (isSupabaseAvailable()) {
    const { data, error } = await supabase
      .from('purchase_orders')
    ...
    const db = await getDb();
    await db.run(
      `UPDATE purchase_orders SET file_name = NULL, file_type = NULL, file_data = NULL WHERE id = ?`,
      [intId]
    );
    return true;
  }
