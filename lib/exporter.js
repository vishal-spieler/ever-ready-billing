...
// Helper to format currency (Using Rs. as jsPDF standard fonts do not support Indian Rupee symbol)
...
export const numberToWords = (amount) => {
  ...
  return word.replace(/\s+/g, ' ').trim() + ' Only';
};

const getStampBase64 = async () => {
  ...
  try {
    ...
    console.error('Failed to fetch stamp image:', err);
    return null;
  }
};
...
export async function exportToExcel({ invoices = [], payments = [], clients = [], quotations = [], companyName = 'EVER READY ENGINEERS' }) {
  ...
  const itemHeaders = [
    'Invoice / Item Name', 'Description', 'HSN Code', 'UOM', 'Qty', 'Rate', 
    'CGST %', 'SGST %', 'Tax Amt', 'Total', 'Details'
  ];
  ...
  invoices.forEach(inv => {
    ...
    parentRow.getCell(10).numFmt = '₹#,##0.00';
    parentRow.height = 22;
    ...
    items.forEach(item => {
      ...
      const taxAmt = taxableSubtotal * ((cgst + sgst) / 100);
...
export async function exportInvoicePDF({ invoice, companySettings }) {
  const stampImg = await getStampBase64();
  const doc = new jsPDF();
  ...
  doc.rect(10, 10, pageWidth - 20, 36); // border box (reduced from 48 to 36)

  // Company logo text / name
  doc.setTextColor(27, 42, 74);
  doc.setFont('Helvetica', 'bold');
  doc.setFontSize(18);
  doc.text(companySettings.name, 15, 18);

  doc.setFont('Helvetica', 'normal');
  doc.setFontSize(8.5);
  doc.setTextColor(80, 80, 80);
  const addrLines = doc.splitTextToSize(companySettings.address, pageWidth - 100);
  doc.text(addrLines, 15, 23);
  doc.text(`Mob: ${companySettings.phone} | Email: ${companySettings.email}`, 15, 33);

  // Invoice label & logo box
  doc.setDrawColor(200, 200, 200);
  doc.setLineWidth(0.5);
  doc.line(pageWidth - 80, 10, pageWidth - 80, 46); // divider line
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(80, 80, 80);
  doc.text(`Invoice No: ${invoice.invoiceNo || invoice.invoice_no}`, pageWidth - 70, 24);
  doc.text(`Date: ${invoice.invoiceDate || invoice.invoice_date}`, pageWidth - 70, 29);
  doc.text(`GSTIN: ${companySettings.gstin}`, pageWidth - 70, 34);
  doc.text(`PAN: ${companySettings.pan}`, pageWidth - 70, 39);

  // Client Details Row
  doc.setFont('Helvetica', 'bold');
  doc.setFontSize(9.5);
  doc.setTextColor(27, 42, 74);
  doc.text('BILL TO:', 10, 52); // pulled up coordinates

  doc.setFont('Helvetica', 'normal');
  doc.setFontSize(9);
  doc.setTextColor(30, 30, 30);
  doc.text(invoice.clientName || 'N/A', 10, 57);
  doc.text(invoice.clientAddress || '', 10, 62);
  doc.text(`${invoice.clientCity || ''} - ${invoice.clientPin || ''}`, 10, 67);
  doc.text(`Mobile: ${invoice.clientMobile || ''}`, 10, 72);
  if (invoice.clientGstin) doc.text(`GSTIN: ${invoice.clientGstin}`, 10, 77);

  // PO details on the right
  doc.setFont('Helvetica', 'bold');
  doc.text('DELIVERY / PO DETAILS:', pageWidth - 90, 52);
  doc.setFont('Helvetica', 'normal');
  doc.text(`PO No: ${invoice.poNo || invoice.po_no || '—'}`, pageWidth - 90, 57);
  doc.text(`PO Date: ${invoice.poDate || invoice.po_date || '—'}`, pageWidth - 90, 62);
  doc.text(`Tax Type: ${invoice.taxType || invoice.tax_type}`, pageWidth - 90, 67);
  ...
  // Line Items Table
  const tableHeaders = [['Sr', 'Material Description', 'HSN Code', 'UOM', 'Qty', 'Rate', 'CGST', 'SGST', 'Tax Amt', 'Total']];
  ...
  // Individual CGST & SGST taxes are shown in each item row, and we display the combined Total Tax.
  const totalTax = (invoice.cgstAmount || 0) + (invoice.sgstAmount || 0) + (invoice.igstAmount || 0);
  if (totalTax > 0) {
    doc.text('Total Tax:', leftAlignLabelX, currentY);
    doc.text(formatCurrency(totalTax), rightAlignX, currentY, { align: 'right' });
    currentY += 6;
  }
  ...
  // Draw Company Stamp
  if (stampImg) {
    doc.addImage(stampImg, 'PNG', sigX + 13, currentY + 4, 22, 22);
  } else {
    doc.setDrawColor(231, 76, 60); // Red
    doc.setTextColor(231, 76, 60);
    doc.setLineWidth(0.5);
    doc.roundedRect(sigX + 2, currentY + 8, 45, 15, 2, 2);
    doc.roundedRect(sigX + 3, currentY + 9, 43, 13, 1.5, 1.5);
    doc.setFontSize(8);
    doc.setFont('Helvetica', 'bold');
    doc.text('EVER READY ENGINEERS', sigX + 5, currentY + 15);
    doc.setFontSize(6);
    doc.text('AUTHORIZED SIGNATORY', sigX + 8, currentY + 20);
  }

  doc.setDrawColor(0, 0, 0); // reset color
  doc.setTextColor(27, 42, 74); // reset color
...
export async function exportReceiptPDF({ payment, invoice, companySettings }) {
  ...
  // Add Company Stamp for Receipt
  const sigX = pageWidth - 65;
  doc.setFont('Helvetica', 'bold');
  doc.setFontSize(8.5);
  doc.setTextColor(27, 42, 74);
  doc.text(`For ${companySettings.name}`, sigX, currentY - 15);
  
  if (stampImg) {
    doc.addImage(stampImg, 'PNG', sigX + 13, currentY - 14, 22, 22);
  } else {
    doc.setDrawColor(231, 76, 60);
    doc.setTextColor(231, 76, 60);
    doc.setLineWidth(0.5);
    doc.roundedRect(sigX + 2, currentY - 13, 45, 15, 2, 2);
    doc.roundedRect(sigX + 3, currentY - 12, 43, 13, 1.5, 1.5);
    doc.setFontSize(8);
    doc.setFont('Helvetica', 'bold');
    doc.text('EVER READY ENGINEERS', sigX + 5, currentY - 6);
    doc.setFontSize(6);
    doc.text('AUTHORIZED SIGNATORY', sigX + 8, currentY - 1);
  }

  doc.setFont('Helvetica', 'normal');
  doc.setFontSize(9);
  doc.setTextColor(120, 120, 120);
  doc.text('This is a computer generated system receipt. No physical signature required.', pageWidth / 2, currentY + 15, { align: 'center' });
  doc.text(`Thank you for doing business with ${companySettings.name}!`, pageWidth / 2, currentY + 20, { align: 'center' });
